...
class YoutubeTest extends TestCase
{
     ...
     */
    public $youtube;
    private $channelId = 'UCp0B9n0YYC8E8bJmS5i4oqw';

    public function setUp()
    {
        $TEST_API_KEY = 'AIzaSyBfevRM4PoY_3iLxpl1tOxjiaIZvSTIbEU';
    ...
    {
        $limit = rand(3, 10);
        $response = $this->youtube->searchChannelVideos('Android', $this->channelId, $limit);
        $this->assertEquals($limit, count($response));
        $this->assertEquals('youtube#searchResult', $response[0]->kind);
        $this->assertEquals('youtube#video', $response[0]->id->kind);
    }
    ...
    {
        $response = $this->youtube->getChannelByName('motorgraph');
        ...
        $this->assertEquals('Motorgraph 모터그래프', $response->snippet->title);
        //add all these assertions here in case the api is changed,
        //we can detect it instantly
        $this->assertObjectHasAttribute('snippet', $response);
        $this->assertObjectHasAttribute('contentDetails', $response);
    ...
    {
        $channel = $this->youtube->getChannelFromURL('http://www.youtube.com/user/motorgraph');

        $this->assertEquals($this->channelId, $channel->id);
        $this->assertEquals('Motorgraph 모터그래프', $channel->snippet->title);
    }
